...
def isi(spiketrain, axis=-1):
    ...
    if (intervals < 0).any():
        warnings.warn("ISI evaluated to negative values. Please sort the input array.")
...
def fanofactor(
    ...
    warn_tolerance: pq.Quantity = 0.1 * pq.ms,
    ...
    def _check_input_spiketrains_durations(
        spiketrains: Union[List[neo.SpikeTrain], List[pq.Quantity], List[np.ndarray]],
    ) -> None:
        if all(isinstance(st, neo.SpikeTrain) for st in spiketrains):
            ...
            if not is_time_quantity(warn_tolerance):
                raise TypeError(
                    f"'warn_tolerance' must be a time quantity, but got {type(warn_tolerance)}"
                )
            durations = np.array(tuple(st.duration for st in spiketrains))
            if np.max(durations) - np.min(durations) > warn_tolerance:
                warnings.warn(
                    f"Fano factor calculated for spike trains of "
    ...
    if isinstance(spiketrains, elephant.trials.Trials):
        ...
        return [
            _compute_fano(
                [
                    list_of_lists_of_spiketrains[trial_no][st_no]
...
def instantaneous_rate(
    ...
    * Summary of the output type:
    ...
    if sampling_period.magnitude < 0:
        raise ValueError(
            f"The 'sampling_period' ({sampling_period}) must be non-negative."
        )
...
def time_histogram(
    spiketrains: Union[List[neo.SpikeTrain], neo.SpikeTrain],
    ...
    def _mean() -> pq.Quantity:
        # 'mean': mean spike counts per spike train.
        return pq.Quantity(
            bin_hist / binned_spiketrain.shape[0], units=pq.dimensionless
        )
