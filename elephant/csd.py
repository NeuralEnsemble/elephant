...
#from .csd.icsd import estimate_csd
...
def estimate_csd(lfp, coords=None, method=None, params={}, cv_params={}):
    ...
    if not isinstance(lfp[0], neo.AnalogSignal):
        print('Parameter `lfp` must be list(neo.AnalogSignal type objects)')
        raise TypeError
    if coords is None:
        coords = []
        for ii in lfp:
    ...
    if method in espen_implented:
        ### DO SCALING HERE!!!


        # THESE CLAUSES SHOULD MOVE TO ICSD

        # if method in icsd_methods and params['diam'] is None:
        #     print("Parameter `diam` must be specified for iCSD methods: {}".format(
        #         ", ".join(icsd_methods)))
        #     raise ValueError

        # if (params['f_type'] is not 'identity') and (params['f_order'] is None):
        #     print("The order of {} filter must be specified".format(params['f_type']))
        #     raise ValueError
    ...
    if method in kernel_methods:
        ...
        ### HOW TO CONVERT TO ANALOG SIGNAL ARRAY ? maintain consistency!
...
def generate_lfp(csd_profile, ele_xx, ele_yy=None, ele_zz=None, xlims=[0.,1.], ylims=[0.,1.], zlims=[0.,1.], res=50):
    ...
    for ii in range(len(pots)):
        ...
        asig = neo.AnalogSignal(pots[ii], sampling_rate=1000*pq.Hz)
        rc.analogsignals = [asig]
        rc.create_relationship()
        lfp.append(asig)
    lfp = neo.AnalogSignalArray(lfp,
    return lfp
