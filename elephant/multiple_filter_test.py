...
(costant rate) or not (one or more points at which the spike increases or decreas
its rate). In this case the output is a list of detected Changing Points (CP).
Essentialy a set of window with different width h (Rh(t)) slide over the time of the
spike [h,T-h]. These generate a deterministic processes that at each time t 
assign the different of spikes count between the left and right side of the window.
If at time t this difference is large 'enough', this can be interpreted like a 
change in the rate of the process in a neighborhood of t.
To give a meaning to 'large enough', a treshold Q is generate using a method
...
def CalcolateTresh(H,T,alfa,Num_sim,dt):    
    ...
    Simulate many of this limit processes(Num_sim), the maxima are collected:
        -first fixed h in H, take the maximum of Lh,t over t in [h,T-h], called m_h
        -second for each simulation take the maximum of m_h over h in H 

    The threshold will be considered the alfa% quantile of this set of maxima.
    -----alfa% quantile of max(h in H)[max(t in T)Lh,t]--------
...
def Gth(t,h,spk):
    ...
    Return
    ------
        g:     scalar, difference of spike count normalized by the variance of the count
    ...
    if ((mu_le>0) & (mu_ri>0)):  #extimeted variance
...
def Rth(dt,h,spk,T,Emp_param): 
    '''
    Given a spike train, generate a deterministic process of the differences,
...
def MultipleFilterAlgorithm(H,spk,T,alfa,Num_sim,Q=None,Emp_param=None,dt=None):
    ...
    C = []      # Matrix of dected point, to be in return
...
# We add to the list of detected ChangingPoint first all the ones detected 
# by the smallest window then using windows with increasing width h,
# in order to avoid FP,we add to the list only those whose h-neighborhood 
# does not include other already listed (with smaller h) changing points.
            ok=1
            if i ==0:
                Ch.append(ci)
            else: 
