...
class KCSD1D_TestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.test_params = {"h": 50.0}

        temp_signals = []
        for ii in range(len(self.pots)):
            temp_signals.append(self.pots[ii])
        self.an_sigs = neo.AnalogSignal(
            np.array(temp_signals).T * pq.mV, sampling_rate=1000 * pq.Hz
        )
    ...
    def test_kcsd1d_estimate(self, cv_params={}):
        ...
        result = CSD.estimate_csd(
            self.an_sigs, method=self.test_method, **self.test_params
        )
        self.assertEqual(result.t_start, 0.0 * pq.s)
        self.assertEqual(result.sampling_rate, 1000 * pq.Hz)
        self.assertEqual(result.times, [0.0] * pq.s)
...
class KCSD2D_TestCase(unittest.TestCase):
    def setUp(self):
        xx_ele, yy_ele = utils.generate_electrodes(
            dim=2, res=9, xlims=[0.05, 0.95], ylims=[0.05, 0.95]
        )
    ...
    def test_kcsd2d_estimate(self, cv_params={}):
        ...
        true_csd = self.csd_profile(
            result.annotations["x_coords"], result.annotations["y_coords"]
        )
        rms = np.linalg.norm(np.array(result[0, :]) - true_csd)
        rms /= np.linalg.norm(true_csd)
        self.assertLess(
            rms, 0.5, msg="RMS " + str(rms) + "between trueCSD and estimate > 0.5"
        )

    def test_moi_estimate(self):
        result = CSD.estimate_csd(
            self.an_sigs, method="MoIKCSD", MoI_iters=10, lambd=0.0, gdx=0.2, gdy=0.2
        )
        self.assertEqual(result.t_start, 0.0 * pq.s)
        self.assertEqual(result.sampling_rate, 1000 * pq.Hz)
        self.assertEqual(result.times, [0.0] * pq.s)
...
class KCSD3D_TestCase(unittest.TestCase):
    def setUp(self):
        xx_ele, yy_ele, zz_ele = utils.generate_electrodes(
            dim=3, res=5, xlims=[0.15, 0.85], ylims=[0.15, 0.85], zlims=[0.15, 0.85]
        )
