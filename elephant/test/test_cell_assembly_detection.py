...
class CadTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.patt1 = [self.patt1_times] + [
            neo.SpikeTrain(
                self.patt1_times + l * pq.s,
                t_start=self.t_start * pq.s,
                t_stop=self.t_stop * pq.s,
            )
            for l in self.lags1
        ]
        self.patt2 = [self.patt2_times] + [
            neo.SpikeTrain(
                self.patt2_times + l * pq.s,
                t_start=self.t_start * pq.s,
                t_stop=self.t_stop * pq.s,
            )
            for l in self.lags2
        ...
        # Binning spiketrains
        self.bin_patt1 = conv.BinnedSpikeTrain(self.patt1, bin_size=self.bin_size)
        ...
        self.elements_msip = [
            self.elements1,
            range(self.n_spk1, self.n_spk1 + self.n_spk2),
            range(self.n_spk1 + self.n_spk2, self.n_spk1 + self.n_spk2 + self.n_spk3),
        ]
        self.occ1 = np.unique(
        ...
        self.lags_msip = [self.output_lags1, self.output_lags2, self.output_lags3]
    ...
    def test_cad_single_sip(self):
        ...
        assert_array_equal(output_single[0]["times"], self.occ1 * self.bin_size)
        # check the lags
        assert_array_equal(
            sorted(output_single[0]["lags"]) * pq.s, self.output_lags1 * self.bin_size
        )
