...
# cache directory.
HAS_CACHE_DIR = (
    "ELEPHANT_DATA_LOCATION" in os.environ
...
class TestDownloadDatasets(unittest.TestCase):
    ...
    def test_invalid_path(self):
        ...
        self.assertIn("ELEPHANT_DATA_LOCATION must be set to either", exception_msg)
        self.assertIn("/invalid/path", exception_msg)

    @patch.dict(os.environ, {"ELEPHANT_DATA_LOCATION": "ftp://invalid.url"}, clear=True)
    ...
    def test_valid_data(self):
        ...
        downloaded_file = download_datasets(repo_path, filepath=None, checksum=None)
    ...
    def test_valid_data_with_path(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            downloaded_file = download_datasets(
                repo_path, filepath=target_file_path, checksum=None
            )
            self.assertTrue(Path(downloaded_file).is_file())
    ...
    def test_valid_data_with_integrity_check(self):
        ...
        expected_checksum = hashlib.md5(open(expected_dataset, "rb").read()).hexdigest()
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            downloaded_file = download_datasets(
                repo_path, filepath=target_file_path, checksum=expected_checksum
            )
            self.assertTrue(Path(downloaded_file).is_file())
            self.assertEqual(target_file_path, downloaded_file)
    ...
    def test_valid_data_existing(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            expected_checksum = hashlib.md5(
                open(downloaded_file, "rb").read()
            ).hexdigest()

            existing_file = download_datasets(
                repo_path, filepath=target_file_path, checksum=expected_checksum
            )
    ...
    def test_valid_data_with_failed_integrity_check(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(ValueError) as error:
                download_datasets(
                    repo_path, filepath=target_file_path, checksum="aaaaaa"
                )
            exception_msg = str(error.exception)
    ...
    @unittest.skipIf(
        not HAS_CACHE_DIR,
        "Not testing since ELEPHANT_DATA_LOCATION is not set to a folder",
    )
    def test_download_with_cache_dir(self):
        ...
        # Expected path of the downloaded dataset
        expected_file_path = (
            Path(os.environ["ELEPHANT_DATA_LOCATION"]) / "dataset-1" / "dataset-1.h5"
        )
    ...
    def test_download_with_cache_dir_with_integrity_check(self):
        ...
        expected_checksum = hashlib.md5(
            open(expected_file_path, "rb").read()
        ).hexdigest()

        downloaded_file = download_datasets(
            repo_path, filepath=None, checksum=expected_checksum
        )
        self.assertTrue(Path(downloaded_file).is_file())
        self.assertEqual(expected_file_path, downloaded_file)
    ...
    def test_download_with_cache_dir_invalid_file(self):
        ...
        with self.assertRaises(ValueError) as error:
            download_datasets(repo_path)
        exception_msg = str(error.exception)
        self.assertIn("ELEPHANT_DATA_LOCATION is set to the local path", exception_msg)
        self.assertIn(f"'{repo_path}' does not exist in that path", exception_msg)
...
class TestLoadData(unittest.TestCase):
    def test_load_data_invalid(self):
        ...
        exception_msg = str(error.exception)
        self.assertIn(
            "not available as downloadable datasets or generated data", exception_msg
        )
    ...
    def test_asset(self):
        ...
        # Compare spike times
        for load_st, expected_st in zip(
            asset_data.spiketrains, downloaded_asset_data.spiketrains
        ):
            assert_allclose(load_st.magnitude, expected_st.magnitude, atol=1e-8)

        # Compare annotations
        for annotation in ("nix_name", "spiketrain_ordering"):
            self.assertEqual(
                downloaded_asset_data.annotations[annotation],
    ...
    def test_unitary_events(self):
        ...
        self.assertTrue(
            all(all(isinstance(st, neo.SpikeTrain) for st in sts) for sts in ue_data)
        )
        ...
        downloaded_ue_data = [
            [st for st in segment.spiketrains]
        ...
        for load_trial, expected_trial in zip(ue_data, downloaded_ue_data):
            for load_st, download_st in zip(load_trial, expected_trial):
                assert_allclose(load_st.magnitude, load_st.magnitude, atol=1e-8)
                self.assertDictEqual(load_st.annotations, download_st.annotations)
