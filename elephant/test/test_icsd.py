...
def potential_of_plane(
    z_j, z_i=0.0 * pq.m, C_i=1 * pq.A / pq.m**2, sigma=0.3 * pq.S / pq.m
):
    """
    ...
    try:
        ...
        print("units of z_j ({}) and z_i ({}) not equal".format(z_j.units, z_i.units))
        raise ae

    return -C_i / (2 * sigma) * abs(z_j - z_i).simplified


def potential_of_disk(
    z_j,
    z_i=0.0 * pq.m,
    C_i=1 * pq.A / pq.m**2,
    R_i=1e-3 * pq.m,
    ...
    try:
        ...
        print(
            "units of z_j ({}), z_i ({}) and R_i ({}) not equal".format(
...
def potential_of_cylinder(
    ...
    try:
        ...
        print(
            "units of z_j ({}), z_i ({}), R_i ({}) and h ({}) not equal".format(
...
def get_lfp_of_cylinders(
    z_j=np.arange(21) * 1e-4 * pq.m,
    z_i=np.array([8e-4, 10e-4, 12e-4]) * pq.m,
    C_i=np.array([-0.5, 1.0, -0.5]) * pq.A / pq.m**3,
    R_i=np.array([1, 1, 1]) * 1e-3 * pq.m,
    ...
    if plot:
        ...
        ax = plt.gca()
        ax.plot(np.zeros(z_j.size), z_j, "r-o")
        ax.barh(np.asarray(z_i - h_i / 2), np.asarray(C_i), np.asarray(h_i), color="r")
        ax.set_ylim(z_j.min(), z_j.max())
        ax.set_ylabel("z_j ({})".format(z_j.units))
        ax.set_xlabel("C_i ({})".format(C_i.units))
...
class TestICSD(unittest.TestCase):
    ...
    def test_StandardCSD_00(self):
        ...
        C_i[7:12:2] += np.array([-0.5, 1.0, -0.5]) * pq.A / pq.m**2
    ...
    def test_StandardCSD_01(self):
        ...
        # current source density magnitude
        C_i = np.zeros(z_i.size) * pq.A / pq.m**2
        C_i[7:12:2] += np.array([-0.5, 1.0, -0.5]) * 1e3 * pq.A / pq.m**2
    ...
    def test_StepiCSD_units_00(self):
        ...
        C_i[7:12:2] += np.array([-0.5, 1.0, -0.5]) * pq.A / pq.m**3
        ...
        # get LFP and CSD at contacts
        phi_j, C_i = get_lfp_of_cylinders(z_j, z_i, C_i, R_i, h_i, sigma, plot)
    ...
    def test_SplineiCSD_00(self):
        ...
        # get LFP and CSD at contacts
        phi_j, C_i = get_lfp_of_cylinders(z_j, z_i_i, C_i_i, R_i_i, h_i_i, sigma, plot)
