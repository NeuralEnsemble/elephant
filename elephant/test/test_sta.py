...
class sta_TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.st0 = SpikeTrain(
            ...
            units='ms', t_stop=self.anasig0.t_stop)
        self.lst = [SpikeTrain(
            [9 * math.pi, 10 * math.pi, 11 * math.pi, 12 * math.pi],
            units='ms', t_stop=self.anasig1.t_stop),
            SpikeTrain([30, 35, 40], units='ms', t_stop=self.anasig1.t_stop)]
    ...
    def test_spike_triggered_average_with_n_spikes_on_constant_function(self):
        ...
        STA = sta.spike_triggered_average(
            anasig, st, (window_starttime, window_endtime))
        a = int(((window_endtime - window_starttime) *
                anasig.sampling_rate).simplified)
        cutout = anasig[0: a]
    ...
    def test_spike_triggered_average_with_shifted_sin_wave(self):
        ...
        STA = sta.spike_triggered_average(
            self.anasig0, self.st0, (-4 * ms, 4 * ms))
        target = 5e-2 * mV
    ...
    def test_usage_of_spikes(self):
        st = SpikeTrain([16.5 * math.pi, 17.5 * math.pi,
                         18.5 * math.pi, 19.5 * math.pi],
                        units='ms', t_stop=20 * math.pi)
        STA = sta.spike_triggered_average(
            self.anasig0, st, (-math.pi * ms, math.pi * ms))
    ...
    def test_one_smaller_nrspiketrains_smaller_nranalogsignals(self):
        ...
        self.assertRaises(ValueError, sta.spike_triggered_average,
                          self.anasig2, self.lst, (-2 * ms, 2 * ms))

    def test_more_spiketrains_than_analogsignals_forbidden(self):
        self.assertRaises(ValueError, sta.spike_triggered_average,
                          self.anasig0, self.lst, (-2 * ms, 2 * ms))
    ...
    def test_impossible_window(self):
        self.assertRaises(ValueError, sta.spike_triggered_average,
                          self.anasig0, self.st0, (-2 * ms, -5 * ms))

    def test_window_larger_than_signal(self):
        self.assertRaises(ValueError, sta.spike_triggered_average,
                          self.anasig0, self.st0,
                          (-15 * math.pi * ms, 15 * math.pi * ms))

    def test_wrong_window_starttime_unit(self):
        self.assertRaises(TypeError, sta.spike_triggered_average,
                          self.anasig0, self.st0, (-2 * mV, 2 * ms))

    def test_wrong_window_endtime_unit(self):
        self.assertRaises(TypeError, sta.spike_triggered_average,
                          self.anasig0, self.st0, (-2 * ms, 2 * Hz))

    def test_window_borders_as_complex_numbers(self):
        self.assertRaises(TypeError, sta.spike_triggered_average, self.anasig0,
                          self.st0, ((-2 * math.pi + 3j) * ms,
                                     (2 * math.pi + 3j) * ms))
    ...
    def test_empty_analogsignal(self):
        anasig = AnalogSignal([], units='mV', sampling_rate=10 / ms)
    ...
    def test_one_spiketrain_empty(self):
        ...
        st = [SpikeTrain(
            [9 * math.pi, 10 * math.pi, 11 * math.pi, 12 * math.pi],
            units='ms', t_stop=self.anasig1.t_stop),
            SpikeTrain([], units='ms', t_stop=self.anasig1.t_stop)]
        with warnings.catch_warnings():
            ...
            """
            STA = sta.spike_triggered_average(self.anasig1,
                                              spiketrains=st,
                                              window=(-1 * ms, 1 * ms))
        assert np.isnan(STA.magnitude[:, 1]).all()

    def test_all_spiketrains_empty(self):
        st = SpikeTrain([], units='ms', t_stop=self.anasig1.t_stop)
        with warnings.catch_warnings(record=True) as w:
            # Cause all warnings to always be triggered.
            warnings.simplefilter("always")
            # Trigger warnings.
            STA = sta.spike_triggered_average(
                self.anasig1, st, (-1 * ms, 1 * ms))
