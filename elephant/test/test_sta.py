...
class sta_TestCase(unittest.TestCase):
    def setUp(self):
        self.asiga0 = AnalogSignal(
            np.array([np.sin(np.arange(0, 20 * math.pi, 0.1))]).T,
            units="mV",
            sampling_rate=10 / ms,
        )
        self.asiga1 = AnalogSignal(
            np.array(
                [
    ...
    def test_spike_triggered_average_with_n_spikes_on_constant_function(self):
        ...
        const = 13.8
        x = const * np.ones(201)
        asiga = AnalogSignal(np.array([x]).T, units="mV", sampling_rate=10 / ms)
        st = SpikeTrain([3, 5.6, 7, 7.1, 16, 16.3], units="ms", t_stop=20)
        window_starttime = -2 * ms
        window_endtime = 2 * ms
        STA = sta.spike_triggered_average(asiga, st, (window_starttime, window_endtime))
        a = int(((window_endtime - window_starttime) * asiga.sampling_rate).simplified)
        cutout = asiga[0:a]
    ...
    def test_spike_triggered_average_with_shifted_sin_wave(self):
        ...
        STA = sta.spike_triggered_average(self.asiga0, self.st0, (-4 * ms, 4 * ms))
        target = 5e-2 * mV
        self.assertEqual(
            np.abs(STA).max().dimensionality.simplified,
    ...
    def test_only_one_spike(self):
        ...
        window_endtime = 5 * ms
        STA = sta.spike_triggered_average(z, st, (window_starttime, window_endtime))
        cutout = z[
            int(((spiketime + window_starttime) * sr).simplified) : int(
                ((spiketime + window_endtime) * sr).simplified
            )
        ]
        cutout.t_start = window_starttime
    ...
    def test_usage_of_spikes(self):
        st = SpikeTrain(
            [16.5 * math.pi, 17.5 * math.pi, 18.5 * math.pi, 19.5 * math.pi],
            units="ms",
            t_stop=20 * math.pi,
        )
    ...
    def test_spike_later_than_analogsignal(self):
        st = SpikeTrain([math.pi, 21 * math.pi], units="ms", t_stop=25 * math.pi)
        self.assertRaises(
            ValueError, sta.spike_triggered_average, self.asiga0, st, (-2 * ms, 2 * ms)
        )
    ...
    def test_window_borders_as_complex_numbers(self):
        self.assertRaises(
            ...
            self.asiga0,
            self.st0,
            ((-2 * math.pi + 3j) * ms, (2 * math.pi + 3j) * ms),
        )
    ...
    def test_one_spiketrain_empty(self):
        ...
        with warnings.catch_warnings():
            ...
            """
            STA = sta.spike_triggered_average(
                self.asiga1, spiketrains=st, window=(-1 * ms, 1 * ms)
            )
    ...
    def test_all_spiketrains_empty(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            cmp_array = AnalogSignal(
                np.array([nan_array, nan_array]).T, units="mV", sampling_rate=10 / ms
            )
...
@unittest.skipIf(
    not hasattr(scipy.signal, "coherence"), "Please update scipy to a version >= 0.16"
)
class sfc_TestCase_new_scipy(unittest.TestCase):
    def setUp(self):
        ...
        self.bst1 = BinnedSpikeTrain(
            self.st0.time_slice(self.anasig3.t_start, self.anasig3.t_stop), bin_size=fs1
        )
        ...
        self.st3 = SpikeTrain(
            np.arange(
                (tlen0.rescale(pq.ms).magnitude * 0.25),
                (tlen0.rescale(pq.ms).magnitude * 0.75),
                50,
            )
            * pq.ms,
            t_start=0 * pq.ms,
        ...
        self.bst5 = BinnedSpikeTrain(self.st3, bin_size=fs0 * 2.0)
    ...
    def test_wrong_input_type(self):
        self.assertRaises(
            TypeError, sta.spike_field_coherence, np.array([1, 2, 3]), self.bst0
        )
        self.assertRaises(TypeError, sta.spike_field_coherence, self.anasig0, [1, 2, 3])
        self.assertRaises(
            ValueError,
    ...
    def test_start_stop_times_out_of_range(self):
        self.assertRaises(
            ValueError, sta.spike_field_coherence, self.anasig1, self.bst0
        )

        self.assertRaises(
            ValueError, sta.spike_field_coherence, self.anasig2, self.bst0
        )

    def test_non_matching_input_binning(self):
        self.assertRaises(
            ValueError, sta.spike_field_coherence, self.anasig0, self.bst1
        )
    ...
    def test_incompatible_spiketrain_analogsignal(self):
        ...
        self.assertRaises(
            ValueError, sta.spike_field_coherence, self.anasig0, self.bst5
        )
        self.assertRaises(
            ValueError, sta.spike_field_coherence, self.anasig0, self.bst6
        )
    ...
    def test_non_binned_spiketrain_input(self):
        ...
        f_ind = np.where(f >= 19.0)[0][0]
        max_ind = np.argmax(s[1:]) + 1

        self.assertEqual(f_ind, max_ind)
        self.assertAlmostEqual(s[f_ind], 1.0, delta=0.01)
    ...
    def test_output_frequencies(self):
        ...
        f_ground_truth = np.linspace(start=0, stop=f_max, num=nfft // 2 + 1) * pq.Hz
    ...
    def test_short_spiketrain(self):
        ...
        with warnings.catch_warnings():
            ...
            warning RuntimeWarning: invalid value encountered in true_divide
              Cxy = np.abs(Pxy)**2 / Pxx / Pyy.
            """
            s1, f1 = sta.spike_field_coherence(self.anasig0, self.bst3, window="boxcar")

            # this spike train has the same spikes as above,
            # but it's shorter than anasig0
            s2, f2 = sta.spike_field_coherence(self.anasig0, self.bst4, window="boxcar")
...
@unittest.skipIf(
    hasattr(scipy.signal, "coherence"), "Applies only for old scipy versions (<0.16)"
)
class sfc_TestCase_old_scipy(unittest.TestCase):
    def setUp(self):
        ...
        def test_old_scipy_version(self):
            self.assertRaises(
                AttributeError, sta.spike_field_coherence, self.anasig0, self.bst0
            )
