...
class FanoFactorTestCase(unittest.TestCase):
    ...
    def test_fanofactor_trials_result(self):
        ...
        for st_idx, result in enumerate(results):
            spiketrains = [
                trial.spiketrains[st_idx]
                for trial in self.test_trials.get_trials_as_list()
            ]
            sp_count = sum([len(spiketrain) for spiketrain in spiketrains])
            self.assertEqual(np.var(sp_count) / np.mean(sp_count), result)
...
class InstantaneousRateTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.n_trials = 36
        cls.n_spiketrains = 5
        block = _create_trials_block(
            n_trials=cls.n_trials, n_spiketrains=cls.n_spiketrains
        )
        cls.block = block
    ...
    def test_instantaneous_rate_errors(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            r"Kernel length .* is longer than binned spike train",
            statistics.instantaneous_rate,
            spiketrains=neo.SpikeTrain(
                pq.Quantity([-5.0, 5.0], "s"), t_start=-5, t_stop=5
            ),
            sampling_period=0.01 * pq.s,
            cutoff=5.0,
            trim=True,
    ...
    @unittest.skip(
        "PR 688: Test replaced since zero instantaneous rate "
    ...
    def test_instantaneous_rate_trim_output_length(self):
        ...
        for kernel in kernels_collected:
            signal_N = int(
                (
                    (self.spike_train.t_stop - self.spike_train.t_start)
                    / self.sampling_period
                ).simplified
            )
            half_kernel_K = int(
                cutoff * (kernel.sigma / self.sampling_period).simplified
            )
    ...
    def test_instantaneous_rate_trim_rate_estimation(self):
        ...
        spikes = np.array([0.100, 0.113, 0.228, 3.901, 8.137, 9.007, 9.837, 9.999])
        spiketrain = neo.SpikeTrain(
            spikes * pq.s, t_start=0.0 * pq.s, t_stop=10.0 * pq.s
        )
        for kernel in kernels_available:
            for center_kernel in (False, True):
                areas = {}
...
class TimeHistogramTestCase(unittest.TestCase):
    ...
    def test_time_histogram_regression_648_single_spiketrain(self):
        ...
        targets = {
            "rate": [2.0, 2.0, 2.0, 2.0, 2.0, 0.0] * pq.Hz,
        ...
        for out_type, res in targets.items():
            ...
            histogram_direct = statistics.time_histogram(
                spiketrain, output=out_type, bin_size=0.5 * pq.s
            )
            histogram_wrapped = statistics.time_histogram(
                [spiketrain], output=out_type, bin_size=0.5 * pq.s
            )

            # Assert direct and wrapped produce same result
            np.testing.assert_array_equal(
                histogram_direct.magnitude, histogram_wrapped.magnitude
            )
