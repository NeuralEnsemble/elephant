...
class SpadeTestCase(unittest.TestCase):
    ...
    def test_different_surrogate_method(self):
        np.random.seed(0)
        random.seed(0)
        spiketrains = stg.StationaryPoissonProcess(
            rate=20*pq.Hz).generate_n_spiketrains(2)
        surr_methods = ('dither_spikes', 'joint_isi_dithering',
                        'bin_shuffling',
                        'dither_spikes_with_refractory_period')
        pv_specs = {'dither_spikes': [[2, 2, 1.0], [2, 3, 0.2]],
                    'joint_isi_dithering': [[2, 2, 0.8]],
                    'bin_shuffling': [[2, 2, 1.0], [2, 3, 0.2], [2, 4, 0.2]],
                    'dither_spikes_with_refractory_period':
                        [[2, 2, 1.0], [2, 3, 0.4]]}
