...
class AnalogSignalThresholdDetectionTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.vm = neo.AnalogSignal(raw_data, units=pq.V, sampling_period=0.1 * pq.ms)
        self.true_time_stamps = [
            0.0123,
            0.0354,
            0.0712,
            0.1191,
...
class AnalogSignalSpikeExtractionTestCase(unittest.TestCase):
    ...
    def test_spike_extraction_waveform(self):
        spike_train = spike_extraction(
            self.vm.reshape(-1), interval=(-1 * pq.ms, 2 * pq.ms)
        )
