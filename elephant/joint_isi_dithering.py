'''
Module to generate surrogates of a spike train by using the joint-ISI dithering.
...
def joint_isi_dithering(st,**kwargs):
    '''
    Implementation of Joint-ISI-dithering for spiketrains that pass the threshold
    of the dense rate, if not a uniform dithered spiketrain is given back.
    The implementation continued the ideas of Louis et al. (2010) and
    Gerstein et al. (2004).
    ...
    **kwargs non-necessary parameters
    ...
    unit
        pq.unit: The unit of the spiketrain in the output.
            Default is pq.s
    window_length
        pq.Quantity: The extent in which the joint-ISI-distribution is calculated.
            Default is 0.06*pq.s
    jisih_bins
        int: The size of the joint-ISI-ditribution will be jisih_bins*jisih_bins.
            Default is 120
    sigma
    ...
    dense_rate
        float: Percentage of isis, that has to be lower then 2*dither, so the
            the joint-ISI-dithering is applied. Else the uniform dithering would
             be applied.
             Default is 0.5
    alternate
        int: determines in which order the ISIs are changed, i.e. if alternate is
            2 first the ISIs with even indices are changed, than those with odd indices.
            Default is 2.
    show_plot
    ...
    print_mode
        boolean: If True, also the way of how the dithered spikes are evaluated
            ...
            Default is False
    use_sqrt
        boolean: if use_sqrt=True a sqrt is applied to the joint-ISI distribution,
            following Gerstein et al. 2004
            Default is False
    method
        string: if 'fast' the entire diagonals of the joint-ISI histograms are used
            if 'window' only the values of the diagonals are used, whose distance is
            lower than dither
            Default is 'fast'
    cutoff
        boolean: if True than the Filtering of the Joint-ISI histogram is limited to
            the lower side by the minimal ISI.
            This can be necessary, if in the data there is a certain dead time,
            which would be destroyed by the convolution with the 2d-Gaussian function.
            Default is True
    number_of_trials
        int: if the number of spikes is lower than this number, the spiketrain is directly
            passed to the uniform dithering.
    ...
    mode (only if print_mode=True)
        string: Indicates, which method was used to dither the spikes.
            ...
            'dither' if the dense_rate was too low and uniform dithering was used.

    '''

    isi,jisih_cumulatives, params=preprocessing_joint_isi_dithering(st,**kwargs)
...
def preprocessing_joint_isi_dithering(st,**kwargs):
    ...
    antidiagonal all points up to the dither-parameter are included, to calculate
    ...
    if np.sum(np.where(isi<2*params['dither'],1.,0.))/len(isi)<params['dense_rate']:
        params['method']='dither'
        return [None], [None], params
    ...
    normalize = lambda v: (v-v[0])/(v[-1]-v[0]) if v[-1]-v[0]>0. else np.zeros_like(v)
    ...
    if params['method']=='fast':
        ...
        jisih_diagonals_cumulatives=np.zeros((jisih_bins,jisih_bins+2*max_change_index))

        for double_index in range(jisih_bins):
            jisih_diagonals_cumulatives[double_index,
                max_change_index:double_index+max_change_index+1]=np.cumsum(np.diagonal(
                flipped_jisih,-jisih_bins+double_index+1))
            jisih_diagonals_cumulatives[double_index,
                double_index+max_change_index+1:double_index+2*max_change_index+1
                ]=np.repeat(jisih_diagonals_cumulatives[double_index,
                double_index+max_change_index],max_change_index)

        jisih_cumulatives=np.zeros((jisih_bins,jisih_bins,2*max_change_index+1))
        for back_index in range(jisih_bins):
            for for_index in range(jisih_bins-back_index):
                double_index=for_index+back_index
                jisih_cumulatives[back_index][for_index]=normalize(jisih_diagonals_cumulatives[
...
def processing_joint_isi_dithering(st,isi,jisih_cumulatives,**params):
    '''
    The main processing function of the joint-ISI dithering. Shall be only used,
    with the output of the preprocessing function.
    ...
    if params['method']=='dither':
        if params['print_mode']:
            return spike_train_surrogates.dither_spikes(st,params['dither']*params['unit'],
        ...
        if params['print_mode']:
            return _joint_isi_dithering_fast(isi,jisih_cumulatives,**params),'jisih'
        else:
            return _joint_isi_dithering_fast(isi,jisih_cumulatives,**params)

    elif params['method']=='window':
        isi,jisih_cumulatives=args
        if params['print_mode']:
            return _joint_isi_dithering_window(isi,jisih_cumulatives,**params),'jisih'
        else:
            return _joint_isi_dithering_window(isi,jisih_cumulatives,**params)
...
def plot_difference_in_joint_isi_distributions(st,dithered_sts,**kwargs):
    ...
    spiketrain against the ones of the dithered spikestrains. The function shows
    three plots:
    ...
    plt.imshow(jisih,origin='lower',extent=(0.,window_length,0.,window_length))
    ...
    plt.figure(figsize=[12.8, 9.6])
    plt.imshow(mean_jisih,origin='lower',extent=(0.,window_length,0.,window_length))
    plt.xlabel('ISI(i+1) in s')
    plt.ylabel('ISI(i) in s')
    plt.title('Joint-ISI-distribution \n Mean Dithered')
    #plt.xlim(left=0.00,right=0.03)
    ...
    plt.figure(figsize=[12.8, 9.6])
    plt.imshow((jisih-mean_jisih)/std_jisih,vmin=-3.,vmax=3.,cmap='nipy_spectral',origin='lower',extent=(0.,window_length,0.,window_length))
    plt.xlabel('ISI(i+1) in s')
    plt.ylabel('ISI(i) in s')
    ...
    **params
    ...
    if params['cutoff']:
        ...
        jisih[start_index:,start_index:]=gaussian_filter(
            jisih[start_index:,start_index:],params['sigma']/params['bin_width'])
    ...
    if params['show_plot']:
        plt.figure(figsize=[12.8, 9.6])
        plt.imshow(jisih,origin='lower',extent=(0.,params['window_length'],
...
def _joint_isi_dithering_fast(isi,jisih_cumulatives,**params):
    ...
    isi
        np.ndarray: The interspike-intervals
    jisih_cumulatives:
        np.ndarray: The cumulatives distribution functions as calculated if __name__ == '__main__':
    ...
    Output:
    dithered_sts
        list of neo.SpikeTrain: A list of len n_surr, each entry is one dithered
            spiketrain.
    '''
    jisih_bins=params['jisih_bins']
    ...
    for surr_number in range(params['n_surr']):
        ...
        dithered_st=first_spike+np.hstack((np.array(0.),np.cumsum(dithered_isi)))
        dithered_st=neo.SpikeTrain(dithered_st*unit,t_stop=t_stop)
        if len(dithered_st)!=number_of_isis+1:
            dithered_st=_correct_artefacts_of_neo(dithered_st,number_of_isis+1)
            ...
            for i in range(k,number_of_isis-1,alternate):
                ...
                if back_index+for_index<jisih_bins:
                    if jisih_cumulatives[back_index][for_index][-1]>0.:
                        step=indices_to_isi[np.where(
                            jisih_cumulatives[back_index][for_index]>random_list[i],
...
def _correct_artefacts_of_neo(st,number_of_spikes):
    ...
    if len(st)<number_of_spikes:
        ...
        st_pure.append(np.random.random()*(t_stop.magnitude-t_start.magnitude)+t_start.magnitude)
        st_pure.sort()
