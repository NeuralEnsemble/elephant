...
def _stretched_metric_2d(
    x, y, stretch, ref_angle, working_memory=None, mapped_array_file=None
):
    r"""
...
class _PMatNeighbors(_GPUBackend):
    ...
    def __init__(self, filter_shape, n_largest, max_chunk_size=None, cuda_threads=None):
        super().__init__(max_chunk_size=max_chunk_size)
    ...
    def pycuda(self, mat):
        ...
        for i_start, i_end in tqdm(
            ...
            if self.cuda_threads:
                ...
                n_threads = min(self.cuda_threads, device.MAX_THREADS_PER_BLOCK)
            else:
                ...
                n_threads = min(max_threads_by_regs, device.MAX_THREADS_PER_BLOCK)
            ...
            if logger.level == logging.DEBUG:
                ...
                logger.debug(
                    f"Memory: shared = {shared_memory}; "
                    f"local = {local_memory}, const = {const_memory}"
                )

                logger.debug(
                    "Maximum per block: threads = "
...
class ASSET(object):
    ...
    def joint_probability_matrix(
        ...
        if (
            isinstance(cuda_threads, tuple)
            and len(cuda_threads) == 2
        ...
        if not (0 < jsf_threads <= 1024) or (
            ...
            raise ValueError(
                "The number of threads in 'cuda_threads' must be"
                "a value > 0 and <= 1024."
            )
        ...
        pmat_neighb_obj = _PMatNeighbors(
            filter_shape=filter_shape, n_largest=n_largest, cuda_threads=pmat_threads
        )
